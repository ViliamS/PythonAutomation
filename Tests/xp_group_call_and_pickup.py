...
6. Callees click at join call button from call notification
...
def execute_caller(agent, mesg, callees_count):
    ...
    agent.wait_until_number_of_participants_in_call(callees_count, group_call=True, fail_on_timeout=False, special_fix_to_reinvite_calee=True)
    for acc in accounts[1:]:
        agent.wait_for_participant_to_appear_in_call(participant_email=acc['username'], group_call=True, timeout=60)
    agent.hang_up_ongoing_call(by_js=True)


def execute_callee(agent, test_accounts):

    agent.join_call_from_call_notification(is_meetup=False)
    agent.wait_for_participant_to_appear_in_call(participant_email=test_accounts[0]['username'], group_call=True)
...
def test(context, caller, *callees):
    ...
    for index, callee in enumerate(callees, start=1):
        t_callees.append(async_call_daemon(execute_callee, callee, test_accounts))
