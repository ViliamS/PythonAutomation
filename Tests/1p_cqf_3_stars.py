...
7. Caller rates call with 3 stars
8. Call quality feedback detailed info request form appears
9. Caller fills up detailed info and submits
...
def test(context, caller):
    ...
    caller.rate_call_quality_feedback_for_call_id_with_n_stars(call_id, 3, scroll_to_elem=False)
    caller.wait_for_quality_feedback_questionnaire_for_call_id(call_id)
    caller.select_video_issue_in_call_quality_feedback_questionnaire_for_call_id(call_id)
    caller.submit_call_quality_feedback_questionnaire_for_call_id(call_id)
